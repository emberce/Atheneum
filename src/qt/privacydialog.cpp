                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zAEM ought to be enough for anybody." - Bill Gates, 2017
    ui->zAEMpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzAEMSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzAEM->setEnabled(false);
        ui->pushButtonMintzAEM->setToolTip(tr("zAEM is currently disabled due to maintenance."));

        ui->pushButtonSpendzAEM->setEnabled(false);
        ui->pushButtonSpendzAEM->setToolTip(tr("zAEM is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zAEMpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzAEM_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zAEM is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zAEM...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zAEM in ") + 
...
void PrivacyDialog::on_pushButtonSpendzAEM_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zAEM is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zAEM
        sendzAEM();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zAEM
    sendzAEM();
}

void PrivacyDialog::on_pushButtonZAEMControl_clicked()
{
    ZAEMControlDialog* zAEMControl = new ZAEMControlDialog(this);
    zAEMControl->setModel(walletModel);
    zAEMControl->exec();
}

void PrivacyDialog::setZAEMControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzAEMSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzAEM()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Atheneum Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zAEMpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zAEM is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " AEM </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zAEM</b>";
    ...
    // use mints from zAEM selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZAEMControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZAEMControlDialog::GetSelectedMints();
    }
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zAEM Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " AEM, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zAEM Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zAEM </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zAEM "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zAEM "));
    ui->labelzAEMAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzAEMSyncStatus->setVisible(fShow);
}
